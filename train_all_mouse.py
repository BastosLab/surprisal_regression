...
SESSION_DIR = '/mnt/data/epoched_mouse_passiveglo'
...
def session_areas(session):
    ...
    timestamps = {
            k: v for (k, v) in zip(possible_areas, raw['muae'][0, :].tolist())
            if len(v)
    }
    del raw
...
def train_ablation(ablations):
    ...
    completed = subprocess.run(['python', 'src/train.py', 'data=mouse',
                                session_spec, area_spec, ablation_spec],
    ...
    for session in tqdm(SESSIONS, desc='Sessions'):
        ...
        for area in tqdm(session_areas(session), desc='Areas', leave=False):
            ...
            with multiprocessing.Pool(8) as pool:
                ms = pool.map(train_ablation, ABLATION_SETTINGS)
                for (ablations, m) in zip(ABLATION_SETTINGS, ms):
                    TRAINING_SESSIONS[session][area].append((ablations, m))

    with open("TRAINING_SESSIONS_MOUSE.json", "w") as f:
