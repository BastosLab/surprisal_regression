...
class TrialwiseLinearRegression(base.PyroModel):
    def __init__(self, ablations=[], hidden_dims=128, num_regressors=5,
                 num_stimuli=4):
        super().__init__()
        ...
        if "selectivity" not in self.ablations:
            self.selectivity_q_alpha = pnn.PyroParam(
                torch.ones(2), constraint=dist.constraints.simplex
            )
            self.register_buffer('selectivity_p_alpha', torch.ones(2))
    ...
    def guide(self, muae, regressors):
        ...
        if "selectivity" in self.ablations:
            selectivity = data.new_zeros(torch.Size((P, B, 2)))
        else:
            alpha = self.selectivity_q_alpha.expand(B, 2)
            selectivity = pyro.sample("stimulus", dist.Dirichlet(alpha))
    ...
    def model(self, muae, regressors):
        ...
        if "selectivity" in self.ablations:
            ...
            alpha = self.selectivity_p_alpha.expand(B, 2)
            selectivity = pyro.sample("stimulus", dist.Dirichlet(alpha))
        ...
        coefficients = torch.cat((selectivity, -adaptation, surprise, time),
                                 dim=-1)
